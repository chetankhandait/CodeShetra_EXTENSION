        ...
        },
        {
            ...
            detectionFunctions: [
                function (node, nodeOld) {
                    ...
                    //          "99% claimed"
                    return /\d+\s*(?:\%|pieces?|pcs\.?|pc\.?|ct\.?|items?)?\s*(?:available|sold|claimed|redeemed|stock)|(?:last|final)\s*(?:article|item)/i.test(node.innerText);
                },
                function (node, nodeOld) {
                    // Return true if a match is found in the current text of the element,
...
detectionFunctions: [
    ...
    function (node, nodeOld) {
        // Additional detection function for the existing scarcity pattern.
        // This could be the existing code for detecting scarcity patterns in English.
        return /\d+\s*(?:\%|pieces?|pcs\.?|pc\.?|ct\.?|items?)?\s*(?:available|sold|claimed|redeemed)|(?:last|final)\s*(?:article|item)/i.test(node.innerText);
    },
    function (node, nodeOld) {
        // Additional detection function for the existing scarcity pattern in German.
        // This could be the existing code for detecting scarcity patterns in German.
        return /\d+\s*(?:\%|stücke?|stk\.?)?\s*(?:verfügbar|verkauft|eingelöst)|letzter\s*Artikel/i.test(node.innerText);
    }
],
infoUrl: brw.i18n.getMessage("patternScarcity_infoUrl"),
...
detectionFunctions: [
    ...
    function (node, nodeOld) {
        // Return true if a match is found for the "Only left in stock" pattern in the current text of the element.
        return /Limited\s*Time\s*deal/i.test(node.innerText);
    },
    function (node, nodeOld) {
        // Additional detection function for the existing scarcity pattern.
        // This could be the existing code for detecting scarcity patterns in English.
